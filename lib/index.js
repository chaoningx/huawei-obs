...
class Index {
   ...
   * @param {String} name
   * @param {String} storagePathOrWriteStream
   ...
   * @param {String} bucketName bucket名
   */
  getObject(name, storagePathOrWriteStream, callback, processCallback, isOverwrite=false, range, bucketName) {
    const headers = {};
    ...
    const writeStream = typeof storagePathOrWriteStream === 'string' ?
      fs.createWriteStream(storagePathOrWriteStream, { flags: isOverwrite ? 'w' : 'wx' }) :
      storagePathOrWriteStream;
