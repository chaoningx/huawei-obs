...
exports.extend = function merge(source, target) {
  const t = {};
...
function getDecimal(num, digit) {
  const n = parseFloat(num);
  if(isNaN(n)) {
    throw new Error('getDecimal method got a invalid param');
  }
  // 保留四位小数
  const pow = Math.pow(10, digit || 4);
  return Math.round(num * pow) / pow;
};

function formatSize(size, isNeedUnit) {
  let str = '';
  let unit = 'B';
  ...
  if(isNaN(size)) {
    throw new Error('formatSize param size invalid');
  }
...
function getAverageSpeed(size, endDate, startDate) {
  const averageSpeed = size / ((endDate.getTime() - startDate.getTime()) / 1000);
  return `${formatSize(averageSpeed)}/s`;
};

exports.getProcessStatus = function (startDate, chunkStartDate, contentLength, receiveSize, chunkSize) {
  const currentTime = new Date();
  const info = {
    contentLength: contentLength,
    receiveSize: receiveSize,
    percent: getDecimal(receiveSize/contentLength),
    instantaneousSpeed: '',
    remainingTime: '', // second
    averageSpeed: '',
    spendTime: ((currentTime - startDate.getTime()) / 1000) + 's', // 花费时间，单位:秒
    time: currentTime
  };

  //速度太快时间上一样，这里修正至少1毫秒
  const difftime = currentTime.getTime() - chunkStartDate.getTime() || 1;
  const averageSpeed = receiveSize / ((currentTime.getTime() - startDate.getTime()) / 1000);
  info.averageSpeed = `${formatSize(averageSpeed)}/s`;

  if(contentLength === 0) {
    return info;
  }

  const speed = chunkSize / difftime * 1000;

  info.remainingTime = `${getDecimal((contentLength - receiveSize) / averageSpeed)}s`;
  info.instantaneousSpeed = `${formatSize(speed)}/s`;
